...
const int TREE_DEPTH = (N < 17) ? N : 2*N-2 - 32; //(N < 18) ? N : 18; // The ternary is just for making testing easier
...
int64_t interval_combine(Interval* intervals, int num_intervals, int* A, int* L, int sizeA, int sizeL);
...
int main() {
    ...
    for (size_t row_idx = 0; row_idx < number_of_rows; row_idx++)
    {
        ...
        for (size_t i = 0; i < A.size; i++) A32.arr[i] = A.arr[i] >> TREE_DEPTH;
        for (size_t i = 0; i < L.size; i++) L32.arr[i] = L.arr[i] >> TREE_DEPTH;
        ...
        // int blockSize = 256;
        // int numBlocks = ceil(interval_size/blockSize);
        ...
        // cudaMemcpy(d_S, S, A32.size * sizeof(int), cudaMemcpyDeviceToHost);
        ...
        free(A32.arr); free(L32.arr); free(al_intervals); free(digit_counts); free(digit_counts_A); free(digit_counts_L);
