...
int main() {
    ...
    for (int row_idx = 0; row_idx < number_of_rows; row_idx++) {
        ...
        BTNode* ABT = newNode(A.size);
        BTNode* LBT = newNode(L.size);
        
        // printf("initialized\n");

        construct_BT(ABT, &A, 2*N-3, 0, A.size);
        construct_BT(LBT, &L, 2*N-3, 0, L.size);

        free(A.arr); free(L.arr);

        printf("counting\n");
        clock_t start_time = clock();
        total_count += BT_combiner(ABT, LBT, 2*N-3);
...
void construct_BT(BTNode* node, Array64Bit* array, int digit, int lower, int upper) {
    // printf("Digit: %d\n", digit);
    if (digit == -1) return; // Base case
    
    // binary search
    int split_idx;
    if (((int64_t)1 << digit) & array->arr[lower]) {
        split_idx = lower;
    } else {
        split_idx = binary_search(array->arr, lower, upper, digit);
    }

    // Construct left and right
    if (split_idx > lower) {
        node->left = newNode(split_idx - lower);
        construct_BT(node->left, array, digit-1, lower, split_idx);
    }
    if (upper > split_idx) {
        node->right = newNode(upper - split_idx);
        construct_BT(node->right, array, digit-1, split_idx, upper);
    }
}
...
int64_t BT_combiner(BTNode* ABT, BTNode* LBT, int digit) {
    if (LBT == NULL || ABT == NULL) return 0;
    if (digit == -1) return ABT->count * LBT->count;

    int64_t result = 0;
    if (ABT->left) result += BT_combiner(ABT->left, LBT->left, digit-1) + BT_combiner(ABT->left, LBT->right, digit-1);
    if (ABT->right) result += BT_combiner(ABT->right, LBT->left, digit-1);
    return result;
}
...
void printBTNode(const BTNode* node) {
    printBT("", node, 0); // Initial call
}

BTNode* newNode(int count) {
    BTNode* node = (BTNode*)malloc(sizeof(BTNode));
    if (node == NULL) {
        fprintf(stderr, "Failed to allocate memory for new BTNode\n");
        exit(EXIT_FAILURE);
    }
    node->count = count;
    node->left = NULL;
