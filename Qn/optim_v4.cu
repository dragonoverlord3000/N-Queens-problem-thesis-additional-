...
// Note: Tree depth is such that the we can reduce to single precision (32bit)
const int TREE_DEPTH = (N == 28) ? 20 : 18; // 2*26-2-18 = 32
const int64_t TREE_MASK = ((int64_t)1 << 32) - 1;
...
int64_t interval_combine(Interval* intervals, int num_intervals, int* A, int* L);
...
int64_t interval_combine64(Interval* intervals, int num_intervals, int64_t* A, int64_t* L);
int removeZeros(Array64Bit X, int* digit_counts);

__global__ void interval_combine_kernel(Interval* intervals, int num_intervals, int* A, int* L, int* S);

int main() {
    char filename[50]; 
    sprintf(filename, "./outputs/output_v4_%d_%d_%d.txt", N, TREE_DEPTH, EARLY_STOPPING);
    ...
    for (size_t row_idx = 0; row_idx < number_of_rows; row_idx++)
    {
        ...
        fprintf(stdout, "A.size: %d, L.size: %d\n", A.size, L.size);

        // Reorder optimization (before sorting)
        int* digit_counts = (int*)calloc(2*N-2, sizeof(int));
        int* digit_counts_A = (int*)calloc(2*N-2, sizeof(int));
        int* digit_counts_L = (int*)calloc(2*N-2, sizeof(int));
        
        digit_counter(A.arr, A.size, 2*N-2, digit_counts_A);
        digit_counter(L.arr, L.size, 2*N-2, digit_counts_L);
        for (size_t i = 0; i < 2*N-2; i++) digit_counts[i] = digit_counts_A[i] * digit_counts_L[i]; // digit_counts_A[i] * digit_counts_L[i] is a good estimate of how much smaller the searchspace becomes at this lvl
        int LENGTH = 2*N-2;
        /* This seems to mess it up */
        // int LENGTH = removeZeros(A, digit_counts);
        // removeZeros(L, digit_counts);
        int permutation[LENGTH];
        createPermutation(digit_counts, 2*N-2, permutation);
        for (size_t i = 0; i < A.size; i++) A.arr[i] = reorder(A.arr[i], permutation, LENGTH);
        for (size_t i = 0; i < L.size; i++) L.arr[i] = reorder(L.arr[i], permutation, LENGTH);

        print_array(digit_counts_A, 2*N-2);
        print_array(digit_counts_L, 2*N-2);
        ...
        fprintf(stdout, "Intervals constructed\n");

        // Reduce to single precision
        Array32Bit A32 = {NULL, A.size, A.capacity, sizeof(int)};
        Array32Bit L32 = {NULL, L.size, L.capacity, sizeof(int)};
        A32.arr = (int*)calloc(A32.size, sizeof(int));
        L32.arr = (int*)calloc(L32.size, sizeof(int));
        for (size_t i = 0; i < A.size; i++) A32.arr[i] = (int)(A.arr[i] & TREE_MASK);
        for (size_t i = 0; i < L.size; i++) L32.arr[i] = (int)(L.arr[i] & TREE_MASK);
        // int64_t actual64 = interval_combine64(al_intervals, interval_size, A.arr, L.arr);
        free(A.arr); free(L.arr);

        // Initialize device memory
        int *d_S, *d_A, *d_L;
        Interval* d_al_intervals;
        cudaMalloc((void **)&d_S, A32.size * sizeof(int));
        cudaMemset(d_S, 0, A32.size * sizeof(int));
        
        cudaMalloc((void **)&d_A, A32.size * sizeof(int));
        cudaMemcpy(d_A, A32.arr, A32.size * sizeof(int), cudaMemcpyHostToDevice);
        cudaMalloc((void **)&d_L, L32.size * sizeof(int));
        cudaMemcpy(d_L, L32.arr, L32.size * sizeof(int), cudaMemcpyHostToDevice);

        cudaMalloc((void **)&d_al_intervals, interval_size * sizeof(Interval));
        cudaMemcpy(d_al_intervals, al_intervals, interval_size * sizeof(Interval), cudaMemcpyHostToDevice);

        int blockSize = 256;
        int numBlocks = ceil(interval_size/blockSize)+1;

        fprintf(stdout, "Combining time :)\n");
        fflush(stdout);
        clock_t start_time = clock();
        interval_combine_kernel<<<numBlocks, blockSize>>>(d_al_intervals, interval_size, d_A, d_L, d_S);
        int* S = (int*)calloc(A32.size, sizeof(int));
        cudaMemcpy(S, d_S, A32.size * sizeof(int), cudaMemcpyDeviceToHost);
        for (size_t i = 0; i < A32.size; i++) total_count += S[i];
        ...
        // int64_t actual32 = interval_combine(al_intervals, interval_size, A32.arr, L32.arr);
        // printf("Actual result 32: %lld\n", actual32);
        // printf("Actual result 64: %lld\n", actual64);

        total_combine_time_interval += (float)(end_time - start_time)/CLOCKS_PER_SEC;
        total_combines_interval++;
        fprintf(stdout, "Total count %llu\t---\tAverage combine time interval: %f\n", total_count, total_combine_time_interval/total_combines_interval);
        fprintf(outputFile, "Total count %llu\t---\tAverage combine time interval: %f\n", total_count, total_combine_time_interval/total_combines_interval);

        // Is needed for testing purposes - otherwise writes take forever
        fflush(stdout);
        fflush(stderr);

        free(A32.arr); free(L32.arr); free(al_intervals); free(digit_counts); free(digit_counts_A); free(digit_counts_L); free(S); free(rowsL);
        cudaFree(d_A); cudaFree(d_L); cudaFree(d_S); cudaFree(d_al_intervals);
    }
...
void appendToArray64Bit(Array64Bit* array, int64_t to_append) {
    if (array->size == array->capacity) {
        ...
        int64_t* new_arr = (int64_t*)realloc(array->arr, new_capacity * sizeof(int64_t));
        if (!new_arr) {
...
void generate_all_rows(int** result_arr, int* temp_arr, int at, int temp_size, int* size) {
    if (temp_size == N/2) {
        int* combination = (int*)malloc(sizeof(int) * (N/2));
...
int* generate_complement(int* arr) {
    int* complement = (int*)malloc(sizeof(int) * (N/2));
...
int binary_search(int64_t* arr, int lower, int upper, int digit) {
    ...
    while (upper - lower > 1) {
        ...
        if (((((int64_t)1) << digit) & arr[mid]) == 0) lower = mid;
        else upper = mid;
    }
    return upper;
}
...
void addInterval(Interval** al_intervals, int* interval_capacity, int* interval_size, int A_left, int A_right, int L_left, int L_right) {
    if (*interval_size == *interval_capacity) {
        ...
        *interval_capacity = (*interval_capacity > 0) ? (*interval_capacity * 2) : 1;
        Interval* new_intervals = (Interval*)realloc(*al_intervals, *interval_capacity * sizeof(Interval));
        if (!new_intervals) {
...
__global__ void interval_combine_kernel(Interval* intervals, int num_intervals, int* A, int* L, int* S) {
    ...
    if (idx < num_intervals) {
        int A_left = intervals[idx].A_left, A_right = intervals[idx].A_right, L_left = intervals[idx].L_left, L_right = intervals[idx].L_right;
        
        for (int j = A_left; j < A_right; j++) {
            for (int k = L_left; k < L_right; k++) {
                if ((A[j] & L[k]) == 0) {
                    atomicAdd(&S[j], 1);
                }
            }
...
// Returns the length of the new integers
int removeZeros(Array64Bit X, int* digit_counts) {
