...
int main() {
    ...
    for (size_t row_idx = 0; row_idx < number_of_rows; row_idx++) {
        ...
        // printf("prefix2count time\n");
        // O(2**p)
        // for (size_t A_idx = 0; A_idx < A.size; A_idx++) printf("Num: %llu, Shift: %d, Prefix: %llu\n", A.arr[A_idx], 2*N-2-p, A.arr[A_idx] >> (2*N - 2 - p));
        for (size_t A_idx = 0; A_idx < A.size; A_idx++) prefix2countA[A.arr[A_idx] >> (2*N - 2 - p)]++;
        // printf("#\n");
        // for (size_t i = 1; i < (1 << p); i++) prefix2countA[i] += prefix2countA[i-1];
        // printf("##\n");

        for (size_t L_idx = 0; L_idx < L.size; L_idx++) prefix2countL[L.arr[L_idx] >> (2*N - 2 - p)]++;
        // printf("###\n");
        // for (size_t i = 1; i < (1 << p); i++) prefix2countL[i] += prefix2countL[i-1];
        // printf("####\n");

        // naive_count += naive_combine(A.arr, L.arr, 0, A.size, 0, L.size);
        // printf("naive_combine: %llu\n", naive_combine(A.arr, L.arr, 0, A.size, 0, L.size));
        
        // printf("L: ");
        // print_array64(L.arr, L.size);
        // printf("A: ");
        // print_array64(A.arr, A.size);
        // printf("A_pref: ");
        // print_array64(prefix2countA, 1 << p);
        // printf("L_pref: ");
        // print_array64(prefix2countL, 1 << p);
    ...
    // printf("Naive count: %llu", naive_count);
